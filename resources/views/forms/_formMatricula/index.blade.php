...
@endif
    ...
    <div class="col-md-5">
        <div class="form-group ">
            <label for="aluno">Nº DE PROCESSO</label>
            <select name="it_idAluno" class="form-control buscarProcesso">
    ...
    <div class="col-sm-5">
        <div class="form-group">
            <label>Turma</label>
            <select name="it_idTurma" class="form-control buscarTurmaMatricula">
                @isset($turma)
        ...
        <div class="form-group">
            @if (!isset($matricula->vc_anoLectivo))
                ...
                <input id="vc_anoLectivo" class="form-control" name="vc_anoLectivo"
                    value="{{ $ano_lectivo_publicado }}" readonly>     <p class="text-danger  " > Atenção: Ano lectivo publicado</p>
            @else
                <input id="vc_anoLectivo" class="form-control" name="vc_anoLectivo"
        ...
        <div class="form-group">
            <div class="file-input ">
                <input name="vc_imagem" type="file" id="file" class="file"
                    value="{{ isset($matricula->vc_imagem) ? $matricula->vc_imagem : '' }}" onchange="
            ...
            <div class="col-lg-12 col-sm-12  ">
                ...
                <div class="card card-outline-info h-100">
                    <div class="card-img-top">
                        <img src="{{ asset($matricula->vc_imagem) }}" id="imageoption" class="grayscale img-fluid mx-auto d-block"
                            alt="card image 3">
                    </div>
                </div>
        ...
        <div class="form-group">
            ...
            <select name="it_idTurma" class="form-control buscarTurmaMatricula">
                ...
                @foreach ($turmas as $turma)
                    @if ($turma->it_qtdeAlunos != $turma->it_qtMatriculados)
                        <option value="{{ $turma->id }}">{{ $turma->vc_nomedaTurma }}|
                            {{ $turma->vc_turnoTurma }} | {{ $turma->vc_cursoTurma }} |
                            {{ $turma->vc_classeTurma }}ª classe</option>
                    @endif
                @endforeach
            </select>
...
<div class="col-sm-3">
    ...
    <div class="file-field input-field col-md-12">
        <div class="form-group">
            <div class="file-input ">
                <input name="vc_imagem" type="file" id="file" class="file" size="50MB image/*" 
                    value="{{ isset($matricula->vc_imagem) ? $matricula->vc_imagem : '' }}" onchange="
                    document.getElementById('imageoption').removeAttribute('src');
                    document.getElementById('imageoption').src = window.URL.createObjectURL(this.files[0])" >
                <label for="file"><i class="fas fa-camera mr-2"></i> Carregar foto</label>
